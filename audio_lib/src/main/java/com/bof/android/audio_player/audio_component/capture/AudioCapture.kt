...
abstract class AudioCapture {
    ...
    // 오디오 캡처 콜백.
    protected var actionOnCapture: ((ShortArray, Int) -> Unit)? = null

    // 현재 오디오 캡처의 상태.
    var recorderState: CaptureState = CaptureState.STOP

    /**
     * 오디오 캡처 사용 전, 초기화 과정.
     */
    open fun prepare(sampleRate: Int = 48000, channelCnt: Int = 1, onCapture: ((ShortArray, Int) -> Unit)? = null) {
        // 초기화 과정.
        this.sampleRate = sampleRate
        this.channelCount = channelCnt
        this.actionOnCapture = onCapture
    }
